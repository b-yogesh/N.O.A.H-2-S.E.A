...
def run_prediction(input):
    net, labels = load_model_with(params, '/Users/rajaathota72/PycharmProjects/zoohackfinal/BirdSongClassifier.pkl', '/Users/rajaathota72/PycharmProjects/zoohackfinal/BirdsSongs.pt')
    predictions, probs = predict(net, labels, input, params)
    return predictions, probs
input = ['/Users/rajaathota72/PycharmProjects/zoohackfinal/testvoices/RustyBlackBird468246.wav']
predictions, probs = run_prediction(input)
print(predictions)

def main():
    import streamlit as st
    st.title("Predict the bird_spieces (voice)- Test")
    import io
    file_buffer = st.file_uploader("upload the voice")
    text_io = io.TextIOWrapper(file_buffer)
    #sound = st.file_uploader("Enter the file","wav")
    st.set_option('deprecation.showfileUploaderEncoding', False)
    x = '/Users/rajaathota72/PycharmProjects/zoohackfinal/testvoices/RustyBlackBird468246.wav'
    if st.button("Recognise"):
        result = run_prediction([x])
        st.audio(x)
        st.success("The bird is identified as {}".format(result[0][0]))
        st.write("The species is endangered. Dont use products from these and you can also report")
